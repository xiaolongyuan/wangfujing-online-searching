 ...
 * @since 1.0.0
 */
@Component("itemPriceChangesFetchOperation")
public class ItemPriceChangesFetchOperation implements IOperation<Void> {
    private boolean running = false;
    @Value("${index.config.priceFetchDuration}")
    private Long priceFetchDuration;
    @Autowired
    ...
    public synchronized Void operate(Operation opRecord)
        ...
        Date after = new Date(currentTimeMillis - this.priceFetchDuration);
        try {
            StringBuilder msg = new StringBuilder();
            this.esService.fetchItemPriceChanges(after, before, Long.parseLong(opRecord.getSid()))
