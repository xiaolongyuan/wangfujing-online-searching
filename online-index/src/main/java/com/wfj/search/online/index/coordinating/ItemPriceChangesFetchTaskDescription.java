 ...
 * @since 1.0.0
 */
public class ItemPriceChangesFetchTaskDescription extends CoordinatingTaskDescription<Operation, JSONObject> {
    private static final SimpleDateFormat DATE_FORMAT = new SimpleDateFormat("yyyyMMddHHmm");
    private final Logger logger = LoggerFactory.getLogger(getClass());
    private final IOperation<Void> itemPriceChangesFetchOperation;

    public ItemPriceChangesFetchTaskDescription(Operation param, IOperation<Void> itemPriceChangesFetchOperation) {
        super("itemPriceChangesFetch", param);
        this.itemPriceChangesFetchOperation = itemPriceChangesFetchOperation;
    }
    ...
    public void executeTask() throws Exception {
        this.itemPriceChangesFetchOperation.operate(this.getParam());
        this.setSuccess(true);
        this.setResultDescription("抓取价格变化完成");
