 ...
 * @since 1.0.0
 */
@Component("facetPriceRangeSearchTask")
public class FacetPriceRangeSearchTask implements SearchTask {
    ...
    public void doSearch(SearchResult searchResult, SolrQuery baseQuery) {
        ...
        if (searchResult != null
                && searchResult.getParams() != null
                && searchResult.getParams().getSelectedRange() != null) {
            // 如果已选参数中存在价格区间，不需要进行价格区间层面分析。
            return;
        }
        // 线上区间facet
        ...
        try {
            ...
            logger.error("加载排序规则失败, 0x530045", e);
            throw new RuntimeTrackingException(new TrackingException(e, "0x530045"));
        }
        ...
        if (rule == null) {
            logger.error("价格区间配置有问题, 0x530046");
            throw new RuntimeTrackingException(new TrackingException("0x530046"));
        }
        doFacetPriceRange(baseQuery, searchResult, rule);
    ...
    private void doFacetPriceRange(SolrQuery query, SearchResult searchResult, IntervalContentPojo rule) {
        ...
        try {
            ...
            logger.error("加载排序规则[{}]明细失败, 0x530047", rule.getSid(), e);
            throw new RuntimeTrackingException(new TrackingException(e, "0x530047"));
        }
        ...
        try {
            ...
            logger.error(msg + ", 0x530048", e);
            throw new RuntimeTrackingException(new TrackingException(e, "0x530048"));
        }
