 ...
 * @since 1.0.0
 */
@Component("alwaysFacetTagSearchTask")
public class AlwaysFacetTagSearchTask extends NewProductsAlwaysFacetSearchTaskBase implements SearchTask {
            ...
            idFFC.stream().map(FacetField.Count::getName)
                    ...
                    .forEach(tagId -> {
                        try {
                            ...
                            logger.error("从ES恢复标签[{}]失败, 0x530090", tagId, e);
                            throw new RuntimeTrackingException(new TrackingException(e, "0x530090"));
                        }
                    });
        } catch (SolrSearchException e) {
            logger.error("facet标签失败, 0x530089", e);
            throw new RuntimeTrackingException(new TrackingException(e, "0x530089"));
        }
