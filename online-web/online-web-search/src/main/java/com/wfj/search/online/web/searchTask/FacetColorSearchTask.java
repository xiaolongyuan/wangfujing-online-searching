 ...
 * @since 1.0.0
 */
@Component("facetColorSearchTask")
public class FacetColorSearchTask implements SearchTask {
    ...
    public void doSearch(SearchResult searchResult, SolrQuery baseQuery) {
        ...
        if (searchResult.getParams().getSelectedColors().getSelected().isEmpty()) {
                ...
                colorAliasFFC.stream().map(FacetField.Count::getName)
                        ...
                        .forEach(colorId -> {
                            try {
                                ...
                                logger.error("从ES恢复色系[{}]失败, 0x530044", colorId, e);
                                throw new RuntimeTrackingException(new TrackingException(e, "0x530044"));
                            }
                        });
            } catch (SolrSearchException e) {
                logger.error("facet颜色失败, 0x530043", e);
                throw new RuntimeTrackingException(new TrackingException(e, "0x530043"));
            }
