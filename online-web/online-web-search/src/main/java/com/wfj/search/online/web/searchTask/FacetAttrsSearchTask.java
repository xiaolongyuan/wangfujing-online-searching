 ...
 * @since 1.0.0
 */
@Component("facetAttrsSearchTask")
public class FacetAttrsSearchTask implements SearchTask {
    ...
    public void doSearch(SearchResult searchResult, SolrQuery baseQuery) {
        ...
        Set<String> selectedPropIds = searchResult.getParams().getSelectedProperties().stream()
                .map(PropertyDisplayPojo::getId).collect(Collectors.toSet());
        try {
            propertyIdFFC = this.itemIAO.facetField(query, "propertyIds_" + channel);
        } catch (SolrSearchException e) {
            logger.error("facet属性失败, 0x530035", e);
            throw new RuntimeTrackingException(new TrackingException(e, "0x530035"));
        }
        propertyIdFFC.stream().map(FacetField.Count::getName)
                .filter(propertyId -> StringUtils.isNotBlank(propertyId) && !"0".equals(propertyId))
                .filter(propertyId -> !selectedPropIds.contains(propertyId))
                .forEach(propertyId -> {
                    try {
                        ...
                        logger.error("从ES恢复属性[{}]失败, 0x530036", propertyId, e);
                        throw new RuntimeTrackingException(new TrackingException(e, "0x530036"));
                    }
                });
        try {
            propertyValueIdFFCs = this.itemIAO.facetPropertyValueIds(query, properties, channel);
        } catch (SolrSearchException e) {
            logger.error("facet属性失败, 0x530037", e);
            throw new RuntimeTrackingException(new TrackingException(e, "0x530037"));
        }
            ...
            counts.stream().map(FacetField.Count::getName).filter(
                    ...
                    .forEach(propertyValueId -> {
                        try {
                            ...
                            logger.error("从ES恢复属性值[{}]失败, 0x530038", propertyValueId, e);
                            throw new RuntimeTrackingException(new TrackingException(e, "0x530038"));
                        }
                    });
            if (values.size() > 0) {
