...
import com.wfj.search.online.web.query.JustBrandFilterQueryDecorator;
 ...
 * @since 1.0.0
 */
@Component("brandFacetCategoryTreeSearchTask")
public class BrandFacetCategoryTreeSearchTask implements SearchTask {
    ...
    @Autowired
    private JustBrandFilterQueryDecorator justBrandFilterQueryDecorator;

    @Override
    public void doSearch(SearchResult searchResult, SolrQuery baseQuery) {
        SolrQuery query = new SolrQuery();
        this.justBrandFilterQueryDecorator.decorator(query, searchResult.getParams());
        ...
        try {
            ...
            for (String catId : catIds) {
                try {
                    ...
                    logger.error("从ES恢复分类[{}]失败, 0x530033", catId, e);
                    throw new RuntimeTrackingException(new TrackingException(e, "0x530033"));
                }
            }
        } catch (SolrSearchException e) {
            logger.error("facet分类失败, 0x530032", e);
            throw new RuntimeTrackingException(new TrackingException(e, "0x530032"));
        }
