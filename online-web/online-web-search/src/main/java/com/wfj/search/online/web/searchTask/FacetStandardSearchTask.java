 ...
 * @since 1.0.0
 */
@Component("facetStandardSearchTask")
public class FacetStandardSearchTask implements SearchTask {
    ...
    public void doSearch(SearchResult searchResult, SolrQuery baseQuery) {
        ...
        if (searchResult.getParams().getSelectedStandards().getSelected().isEmpty()) {
                ...
                standardIdFFC.stream().map(FacetField.Count::getName)
                        ...
                        .forEach(standardId -> {
                            try {
                                ...
                                logger.error("从ES恢复规格[{}]失败, 0x530050", standardId, e);
                                throw new RuntimeTrackingException(new TrackingException(e, "0x530050"));
                            }
                        });
            } catch (SolrSearchException e) {
                logger.error("facet规格失败, 0x530049", e);
                throw new RuntimeTrackingException(new TrackingException(e, "0x530049"));
            }
