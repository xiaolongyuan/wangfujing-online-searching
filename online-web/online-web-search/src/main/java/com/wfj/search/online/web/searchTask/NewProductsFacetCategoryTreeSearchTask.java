...
import com.wfj.search.online.web.query.OnSellSinceFilterDecorator;
 ...
 * @since 1.0.0
 */
@Component("newProductsFacetCategoryTreeSearchTask")
public class NewProductsFacetCategoryTreeSearchTask implements SearchTask {
    ...
    @Autowired
    private OnSellSinceFilterDecorator onSellSinceFilterDecorator;
    ...
    public void doSearch(SearchResult searchResult, SolrQuery baseQuery) {
        ...
        this.justQQueryDecorator.decorator(query, searchResult.getParams());
        this.onSellSinceFilterDecorator.decorator(query, searchResult.getParams());
        ...
        try {
            ...
            for (String catId : catIds) {
                try {
                    ...
                    logger.error("从ES中恢复分类[{}]信息失败, 0x530056", catId, e);
                    throw new RuntimeTrackingException(new TrackingException(e, "0x530056"));
                }
            }
        } catch (SolrSearchException e) {
            logger.error("facet分类失败, 0x530055", e);
            throw new RuntimeTrackingException(new TrackingException(e, "0x530055"));
        }
