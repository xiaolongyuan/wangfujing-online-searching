 ...
 * @since 1.0.0
 */
@Component("facetTagSearchTask")
public class FacetTagSearchTask implements SearchTask {
    ...
    public void doSearch(SearchResult searchResult, SolrQuery baseQuery) {
        ...
        if (searchResult.getParams().getSelectedTags().getSelected().isEmpty()) {
                ...
                idFFC.stream().map(FacetField.Count::getName)
                        ...
                        .forEach(tagId -> {
                            try {
                                ...
                                logger.error("从ES恢复标签[{}]失败, 0x530052", tagId, e);
                                throw new RuntimeTrackingException(new TrackingException(e, "0x530052"));
                            }
                        });
            } catch (SolrSearchException e) {
                logger.error("facet标签失败, 0x530051", e);
                throw new RuntimeTrackingException(new TrackingException(e, "0x530051"));
            }
